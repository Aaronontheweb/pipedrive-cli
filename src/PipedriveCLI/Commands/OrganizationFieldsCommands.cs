        ...
        listCommand.SetHandler(async (search, customOnly, json) =>
        {
            try
            {
                ...
                var response = await JsonOutputHelper.FetchAsync(
                    ...
                    () => apiClient.GetOrganizationFieldsAsync());

                if (response?.Success == true)
                {
                    var fields = (response.Data ?? new List<OrganizationField>()).AsEnumerable();
                    ...
                    if (json)
                    {
                        JsonOutputHelper.Write(response, ApiJsonContext.Default.PipedriveResponseListOrganizationField);
                        return;
                    }
